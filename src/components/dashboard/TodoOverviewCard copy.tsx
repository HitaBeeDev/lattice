...
export default function TodoOverviewCard({
  ...
  const pendingPct =
    visibleTasks.length > 0
      ...
      : 0;
  const freePct =
    emptyCount > 0 ? Math.round((emptyCount / MAX_CARD_TASKS) * 100) : 0;
  ...
  return (
    <section className="col-span-1 col-start-4 row-span-4 row-start-1 flex h-full w-full flex-col overflow-hidden rounded-[1.35rem] bg-[linear-gradient(180deg,#edf9fc_0%,#d7eef5_100%)] px-5 pb-4 pt-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.6)]">
      <button
        ...
        className="rounded-[1rem] text-left transition-opacity hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0a1929]/20"
      ...
      >
        ...
        <div className="flex items-end gap-2 mt-4">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1.5 text-[0.72rem] text-[#3d454c]">In Progress</p>
          </div>
          ...
          <div className="w-[17.5%]">
            ...
            <p className="mt-1.5 text-[0.72rem] text-[#3d454c]">Remaining</p>
          </div>
        </div>
      </button>

      <div className="relative flex flex-col flex-1 min-h-0 mt-8">
              ...
              return (
                ...
                >
                  <button
                    type="button"
                    onClick={handleOpenTasks}
                    className="flex w-full items-center gap-3 rounded-[1rem] text-left transition-colors hover:bg-white/[0.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/20"
          ...
          <div className="mt-4 flex items-center justify-between rounded-[1.05rem] border border-white/8 bg-white/[0.04] px-3 py-2 text-[0.7rem] text-white/52">
            ...
            {hiddenTaskCount > 0 ? (
              <button
                type="button"
                onClick={handleOpenTasks}
                className="text-[0.7rem] text-white/68 underline underline-offset-2 transition-colors hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/20"
              >
                Check full todos...
              </button>
            ) : (
